...
export default function SalesOrderForm({ order, onClose, onSuccess }: Props) {
  ...
  const { data: nextNumberData } = useQuery<{ nextNumber: string }>({
    queryKey: ["/api/orders/next-number"],
    enabled: !isEditMode,
  });

  // Filter customers only (not vendors)
  ...
  useEffect(() => {
    if (!isEditMode && nextNumberData?.nextNumber) {
      form.setValue("orderNumber", nextNumberData.nextNumber);
    }
  }, [nextNumberData, isEditMode, form]);

  useEffect(() => {
    console.log("SalesOrderForm useEffect triggered - order:", order?.id, "isEditMode:", isEditMode);
    ...
    if (order && isEditMode) {
      ...
      if (order.id) {
        ...
        fetch(`/api/orders/${order.id}/line-items`, {
          ...
          .then((res) => {
            console.log("Line items response status:", res.status);
            return res.json();
          })
          .then((items) => {
            console.log("Line items received:", items);
            if (items && Array.isArray(items) && items.length > 0) {
              ...
              console.log("Setting mapped line items:", mappedItems);
              setLineItems(mappedItems);
            } else {
              console.log("No items found, adding default empty row");
        ...
        console.log("No order ID, adding default empty row");
        setLineItems([createEmptyLineItem()]);
        setLineItemsLoaded(true);
      }
    } else if (!isEditMode) {
      console.log("Not edit mode, adding default empty row");
      setLineItems([createEmptyLineItem()]);
      setLineItemsLoaded(true);
    }
  ...
  return (
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-start justify-center overflow-y-auto py-4">
      <Card className="w-full max-w-6xl mx-4" data-testid="sales-order-form">
